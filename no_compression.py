...
class NestedModel(TlvModel):
    cert = BytesField(0x84)
    chain = BytesField(0x85)
    publication = BytesField(0x86)
...
def encode_cert():
    ...
    for key,val in certDict.items():
        ...
        for comp in val[1]:
            ...
            for ch in comp:
                s+= ch
    trustSchemaModel.inner.cert = bytes(s.encode())
...
def encode_pub():
    ...
    trustSchemaModel.inner.publication = bytes(s.encode())
